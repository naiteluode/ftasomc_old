...
import forms
from ops.models import UserProfile, Hosts
        ...
        if user is not None:
            try:
                if django.utils.timezone.now() > user.userprofile.valid_begin_time and django.utils.timezone.now()  < user.userprofile.valid_end_time:
                    ...
                    return render(request,'login.html',{'login_err': '账号过期，请联系管理员!'})
            except ObjectDoesNotExist:
                    return render(request,'login.html',{'login_err': u'账户不存在,请先登录后台面创建OMC账户!'})
...
def hosts_multi(request):
    ...
    return render(request,'hosts/hosts_multi.html',{'login_user':request.user,
...
@login_required
def crontab(request):
    return  render(request,'hosts/crontab.html',{'active_node':'/hosts/crontab/'})
...
def hosts_multi_filetrans(request):
    ...
    return render(request,'hosts/hosts_multi_files.html',{'login_user':request.user,
                                              'recent_tasks': recent_tasks,
...
def host_detail(request):
    ...
    return  render(request, 'hosts/host_detail.html', {'all_hosts':all_hosts,
...
def user_audit(request,user_id):
    ...
    return  render(request,'hosts/user_audit.html',{
...
def multi_task_log_detail(request,task_id):
    ...
    return render(request,'hosts/multi_task_log_detail.html',{'log_obj':log_obj})
...
@login_required	
def assets_loc(request):
    Host_list = Hosts.objects.all()
    return render_to_response('assets_loc.html',{'Host_list':Host_list})
