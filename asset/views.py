...
from ftasomc.mysql import db_operate
...
def host_list_manage(request,id=None):
    ...
    if id:
        ...
        page_name = '编辑主机'
        db = db_operate() 
        sql = 'select ip from omc_hostlist where id = %s' % (id)
        ret = db.mysql_command(settings.cobblerdb,sql)
    else:
        host_list = HostList()
        action = 'add'   
        page_name = '新增主机'
    ...
    return render_to_response('asset_host_manage.html',
           {"form": form,
            "page_name": page_name,
            "action": action,
...
def host_list(request):
    ...
    return render_to_response('asset_host_list.html', {'all_host_list': all_host, 'page': page, 'paginator':paginator})
...
def get_server_asset(request):
    ...
    if request.method == 'GET':
        ...
        if action == 'flush':
            ...
            db = db_operate() 
            host_ret = db.mysql_command(settings.cobblerdb,hostlist_sql)
            server_ret = db.mysql_command(settings.cobblerdb,server_sql)
