...
function getCpuStatus(){
    ...
    if [[ $CPU_Kernels < 2 ]];then
        echo "=> [warn] CPU核心数少于建议值2!"
    else
        echo "=> [ok] 正常!"
    fi
}
...
function getMemStatus(){
    ...
    if [ `echo "${report_MemUsedPercent} 75" | awk '{if($1>$2) {print 0} else {print 1}}'`  -eq 0 ];then
        echo "=> [warn] 内存使用率过高!"
    else
        echo "=> [ok] 正常!"
    fi
...
function getDiskStatus(){
    ...
    echo "硬盘总容量: $report_DiskTotal 硬盘剩余: $report_DiskFree 硬盘使用率: $report_DiskUsedPercent%"
    if [ `echo "${report_DiskUsedPercent} 80" | awk '{if($1>$2) {print 0} else {print 1}}'`  -eq 0 ];then
        echo "=> [warn] 内存使用率过高!"
    else
        echo "=> [ok] 正常!"
    fi
...
function getMaxFileConnection(){
    echo "[ 文件连接数检查 ]"
    maxFileConn=$(ulimit -a | grep open | awk '{print $NF}')
    echo "最大文件连接数: $maxFileConn"
    if [ `echo "${maxFileConn} 65535" | awk '{if($1<$2) {print 0} else {print 1}}'`  -eq 0 ];then
        echo "=> [warn] 最大文件连接数小于65535!"
    else
        echo "=> [ok] 正常!"
    fi
}
